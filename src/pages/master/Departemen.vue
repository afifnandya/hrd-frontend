<template>
    <div class="p-10 bg-white">
        ...
        <DataTable ref="dt" :value="departments" :loading="loading" data-key="id" :paginator="true" :rows="10" paginator-template="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown" :rows-per-page-options="[5, 10, 25]"
    
            current-page-report-template="Showing {first} to {last} of {totalRecords} Divisi" responsive-layout="scroll" scroll-height="400px" scrollable>
    
            <Column field="no" :header="$t('idDepartement')" :sortable="true" header-style="width:14%; min-width:10rem;">
...
<template #body="slotProps">
    <Button icon="pi pi-pencil" class="mr-2 p-button-rounded p-button-success" @click="showEditModal(slotProps.data)" />
    
    <Button icon="pi pi-trash" class="mt-2 p-button-rounded p-button-warning" @click="showDeleteModal(slotProps.data)" />
</template>
      </Column>
    </DataTable>
...
<template #footer>
    <Button :label="$t('cancel')" icon="pi pi-times" class="p-button-text" @click="showModal.add = false" />
    
    <Button :label="$t('save')" icon="pi pi-check" class="p-button-text" :disabled="!createDepartment.name" @click="tambahDepartment" />
</template>
    </Dialog>
...
<template #footer>
    <Button :label="$t('cancel')" icon="pi pi-times" class="p-button-text" @click="showModal.edit = false" />
    
    <Button :label="$t('save')" icon="pi pi-check" class="p-button-text" @click="editData()" />
</template>
    </Dialog>
...
<template #footer>
    <Button label="No" icon="pi pi-times" class="p-button-text" @click="showModal.delete = false" />
